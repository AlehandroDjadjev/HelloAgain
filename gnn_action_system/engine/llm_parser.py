...
class QwenPromptParser:
    ...
    def _normalize_plan(cls, mode: str, raw_plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized_prompt_attrs = cls._normalize_prompt_attributes(
            prompt_context.get("all_relevant_attributes") or payload.get("prompt_affected_attributes")
        )
        normalized_desired_attrs = cls._normalize_runtime_attributes(
            ...
            or payload.get("desired_attributes")
            or payload.get("desired_positive_attributes"),
            limit=2,
        )
        normalized_opposite_attrs = cls._normalize_runtime_attributes(
            ...
            or payload.get("opposite_attributes")
            or payload.get("solution_attributes")
            or payload.get("counter_attributes"),
            limit=2,
        )
        ...
        present_updates = {item["attribute"] for item in normalized_updates}
        for item in normalized_prompt_attrs:
            if (
                not item["should_update_user"]
                or item["attribute"] in present_updates
                or item["attribute"] in new_attribute_names
            ):
                continue
            normalized_updates.append(
                {
                    ...
                    "delta": None,
                    "reason": item["reason"] or "Derived from prompt context.",
                    "explicit_decay": item["explicit_decay"],
                }
            )
            present_updates.add(item["attribute"])
        ...
        return {
            ...
            "prompt_context": {
                ...
                "all_relevant_attributes": normalized_prompt_attrs,
    ...
    def parse(self, *, mode: str, system_prompt: str, user_prompt: str) -> Dict[str, Any]:
        raw_text = self._client.generate(system_prompt=system_prompt, user_prompt=user_prompt)
        parsed = self._extract_json(raw_text)
        normalized = self._normalize_plan(mode=mode, raw_plan=parsed)
